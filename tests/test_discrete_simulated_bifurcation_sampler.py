...
# TO DO, REPLACE SimulatedAnnealingSampler by DiscreteSimulatedBifurcationSampler, up to parameter accommodation. Test additional parameters.
...
class TestSchedules(unittest.TestCase):
    def test_schedules(self):
        ...
        num_reads = 10
        a_schedule = np.arange(0,3)/2
        # TO DO, SET INITIAL CONDITION THEN CHECK IDENTICAL OUTCOMES
        for schedule_type in [None, "linear"]:
            if schedule_type is None:
                resp = sampler.sample_ising(
                    h, J, num_reads=num_reads, num_sweeps=len(a_schedule)
                )
            else:
                resp = sampler.sample_ising(
